...
parser.add_argument("--total_duration", type=float, help="total duration of the reach, including holding times",
                    default=1.5)
parser.add_argument("--hold_duration", type=float, help="hold duration", default=0.25)
parser.add_argument("--sigma", type=float, help="single-unit noise intensity", default=5e-3)
parser.add_argument("--noisy_ics", type=float, help="noise intensity for RNN hidden layer initial condition",
                    default=1.)
parser.add_argument("--reset_radius", type=float, help="noise intensity for arm initial condition", default=0.)
...
parser.add_argument("--size", type=tuple, help="size of the network (in, h1, h2, out)", default=(7, 100, 100, 2))
...
def plot_trajectories(net, dataset, outfile_name=None, n_reals=5):
    fig, axes = plt.subplots(nrows=2, ncols=2,
                             figsize=(2 * 45 * units_convert['mm'], 2 * 45 / 1.25 * units_convert['mm']))
    ...
    with torch.no_grad():
        for _ in range(n_reals):
            for i in range(dataset.n_targets):
                ...
                # plot velocities
                pred_vel = pred[0, :, 2:4]
                axes[0, 1].plot(args.dt * np.arange(len(pred_vel)), np.linalg.norm(pred_vel, axis=1),
                                color=colors[i])
                ...
                # plot controls
                axes[1, 1].plot(args.dt * np.arange(len(pred_f)), np.linalg.norm(controls[0, :, :], axis=-1),
                                color=colors[i])
    axes[0, 0].axis('off')
...
DATADIR = "../data/point-mass-arm-with-feedback-noisy-reset-noisy-holds"
RESULTDIR = "../results/point-mass-arm-with-feedback-noisy-reset-noisy-holds"
...
dataset = HoldsDataset(n_targets=args.n_targets, total_duration=args.total_duration, dt=args.dt,
                       distance=0.07, hold_start=args.hold_duration, hold_end=args.hold_duration, sigma=args.sigma,
                       context='arm')
...
loss_fn = HoldsLoss(args.gamma_v, args.gamma_f, args.lambda_ctrl, args.lambda_rate,
                    args.hold_duration, args.hold_duration, args.dt)
...
for t in range(epochs):
    for X, y in dataloader:
        ...
        if t % 500 == 0:
            print(
                f"Epoch {t}: loss = {loss.item():>10e},  loss_Ctrl = {loss_ctrl.item():>10e}, loss_rate = {loss_rate.item():>10e}")
